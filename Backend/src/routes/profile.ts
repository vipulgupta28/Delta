...
//@ts-ignore
router.post('/handleProfileChanges', upload.fields([
...
//@ts-ignore
router.get('/getProfileChanges/:userId', async (req: Request, res: Response) => {
...
//@ts-ignore
router.post('/handleFollow', async (req: Request, res: Response) => {
...
//@ts-ignore
router.get('/getMutuals/:userId', async (req: Request, res: Response) => {
  const { userId } = req.params;
  ...
  try {
    ...
    const { data: mutualUsers, error: userError } = await supabase
      .from('users')
