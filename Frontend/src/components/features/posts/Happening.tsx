...
const handleChangeForStory = (e: React.ChangeEvent<HTMLTextAreaElement>) =>{
...
useEffect(() => {
  ...
  const fetchMutuals = async () => {
    try {
      const response = await api.get(`/getMutuals/${userId}`);
      setMutuals(response.data.mutuals);
      setFilteredMutuals(response.data.mutuals);
    } catch (err) {
...
useEffect(() => {
  async function getIndiaNews(): Promise<void> {
    try {
      const response = await fetch(
        `https://newsapi.org/v2/top-headlines?country=in&pageSize=5&apiKey=${API_KEY}`
      );
...
useEffect(() => {
 ...
 useEffect(() => {
  ...
  const StorePosts = async () => {
    ...
    try {
      const response = await api.post("/store-posts", formData, {
...
>
  ...
  <div>
    <textarea
      rows={5}
      value={content}
      onChange={handleChangeForStory}
      placeholder="What's the story? (max 50 words)"
  ...
  className={`p-6 mb-6 rounded-xl bg-[#121212] transition-colors ${
    post.username === "vipul" ? "border-zinc-800" : "border-zinc-800"
  } border`}
>
...
{isGalleryOpen && (
  ...
  {isShareModalOpen && (
    <>
      ...
      <div className="hidden sticky lg:block w-72 flex-shrink-0 space-y-4">
  {/* Search input */}
